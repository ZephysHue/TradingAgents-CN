...
MODEL_COMPLETIONS_URL = "http://127.0.0.1:1234/v1/completions"
...
def _clean_model_completion_text(text: str) -> str:
...
def run_local_model_analysis(chosen_rows: dict[str, dict[str, Any]], development_market_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    out: dict[str, Any] = {
        ...
        "completion_url": MODEL_COMPLETIONS_URL,
        "warning_flags": WARNING_FLAGS,
    ...
    prompt_payload = {
        ...
        "development_market_summary": {
            "stage": development_market_summary.get("stage"),
            "symbols": development_market_summary.get("symbols"),
            "tiers": development_market_summary.get("tiers", {}),
        },
        "selected_families": {
            family: {
                "param_id": row["param_id"],
                "trade_count": int(row["trade_count"]),
                "win_rate": round(_safe_float(row["win_rate"]), 2),
                "net_expectancy_bps": round(_safe_float(row["net_expectancy_bps"]), 2),
                "profit_factor": round(_safe_float(row["profit_factor"]), 2),
                "max_drawdown_pct": round(_safe_float(row["max_drawdown_pct"]), 2),
            }
    ...
    request_payload = {
        ...
        "temperature": 0.0,
        "max_tokens": 192,
        "prompt": "Return compact JSON only. No markdown. No preamble. "
        + json.dumps(prompt_payload, ensure_ascii=False),
    }
    out["request_payload"] = request_payload
    try:
        completion_response = requests.post(MODEL_COMPLETIONS_URL, json=request_payload, timeout=90)
        out["completion_status_code"] = completion_response.status_code
        try:
            completion_body = completion_response.json()
            out["completion_body"] = completion_body
            completion_text = str(completion_body.get("choices", [{}])[0].get("text", ""))
            out["completion_text"] = completion_text
            cleaned_text = _clean_model_completion_text(completion_text)
            out["completion_text_clean"] = cleaned_text
            try:
                out["completion_json_candidate"] = json.loads(cleaned_text)
            except json.JSONDecodeError:
                pass
        except Exception:
            out["completion_body_text"] = completion_response.text
    except Exception as exc:  # pragma: no cover - network failure path
        out["completion_error"] = repr(exc)
    out["elapsed_seconds"] = round(time.time() - started, 3)
    return out
...
def write_morning_summary(
    ...
    lines = [
        ...
        f"- /v1/completions: {model_analysis.get('completion_status_code', model_analysis.get('chat_status_code', model_analysis.get('completion_error', model_analysis.get('chat_error', 'not_run'))))}",
        "",
        "## 各策略族结果",
        "",
